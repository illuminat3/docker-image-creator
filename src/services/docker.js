...
async function buildAndPush({ repo, dockerfile, imageName }) {
  ...
  const tmpDir = path.join(
    os.tmpdir(),
    `dic-${crypto.randomBytes(8).toString("hex")}`,
  );
  ...
  try {
    ...
    await run(
      "docker",
      ["login", REGISTRY_URL, "-u", REGISTRY_USERNAME, "--password-stdin"],
      {
    ...
    if (
      !dockerfilePath.startsWith(tmpDir + path.sep) &&
      dockerfilePath !== tmpDir
    ) {
